...
const views = {
  ...
  Finance() {
    ...
    return `${pageHeader('Finance Operations', 'Track obligations, evidence, approvals, and payment instructions—without Wave holding funds.', '<button class="primary-btn" id="uploadInvoice">＋ Add invoice</button>')}
      ...
      <section class="finance-workspace"><article class="module-card invoice-card"><div class="module-card-head"><div><h3>Invoice queue</h3><p>Every instruction remains human-approved and evidence-linked.</p></div></div><div class="table-head finance-head"><span>Vendor</span><span>Deliverable</span><span>Amount</span><span>Status</span><span>Action</span></div>${visible.map((invoice) => `<button class="table-row finance-row ${selected?.id === invoice.id ? 'active' : ''}" data-select-invoice="${invoice.id}"><span><i class="company-avatar">${invoice.vendor[0]}</i><b>${escapeHtml(invoice.vendor)}</b></span><span>${escapeHtml(invoice.deliverable)}<small>${escapeHtml(invoice.reference || 'No reference')} · Due ${escapeHtml(invoice.due)}</small></span><span><b>${money(invoice.amount)}</b></span><span><em class="status-pill ${invoice.status.toLowerCase().replaceAll(' ', '-')}">${escapeHtml(invoice.status)}</em></span><span>${invoice.status === 'Needs approval' || invoice.status === 'Awaiting second approval' ? `<span class="approve-btn" data-approve-invoice="${invoice.id}">Approve</span>` : 'View →'}</span></button>`).join('') || emptyState('$', 'No invoices match', 'Choose another status or add a new invoice.')}</article>
...
function attachModuleEvents(page) {
  ...
  $$('[data-select-invoice]').forEach((button) => button.addEventListener('click', (event) => { if (event.target.dataset.approveInvoice) return; currentInvoiceId = Number(button.dataset.selectInvoice); navigate('Finance'); }));
